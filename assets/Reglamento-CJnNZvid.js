import{b as G,u as _,r as U,A as V,j as e,L as P}from"./index-vauxcNtK.js";import{N as W,C as d,b as l,c as n,d as X,a as c}from"./card-CMMaxxR6.js";import{M as Y,F as J}from"./footer-COBFWtpt.js";import{B as u}from"./button-DjEQfLGY.js";import{B as z}from"./badge-CwGzvRPk.js";import{S as D,l as K}from"./Logo-DO7twlU1.js";import{C as Q,F as q}from"./file-text-nxEoEMZ5.js";import{U as A}from"./users-DzPWuiJn.js";import{A as Z}from"./arrow-left-CJSUP_AX.js";const ce=()=>{const{eventId:m}=G(),{activeEvent:a,activeEventId:g,events:j,setActiveEventId:b}=_();U.useEffect(()=>{m&&j.some(s=>s.id===m)&&g!==m&&b(m)},[g,m,j,b]);const R=new Date(a.dateTime),T=new Date(a.registrationCloseDateTime),v=R.toLocaleString("es-HN",{dateStyle:"long",timeStyle:"short"}),I=T.toLocaleDateString("es-HN",{dateStyle:"long"}),o=!!a.allowMultipleDistances,f=o?"Pruebas":"Distancias",N=a.courseType==="open_water"?V:`L. ${a.price}.`,y=[{icon:e.jsx(Q,{className:"h-6 w-6"}),title:o?"Orden de Competencia":"Salida Escalonada",description:o?"La organización anunciará cada prueba y serie antes de su salida.":"Las salidas se realizarán por categorías de forma escalonada para garantizar la seguridad de todos los participantes."},{icon:e.jsx(A,{className:"h-6 w-6"}),title:"Límite de Participantes",description:`${a.capacityLimit?`Máximo ${a.capacityLimit} participantes.`:"Sin límite de participantes."} Las inscripciones se cerrarán el ${I}.`},{icon:e.jsx(D,{className:"h-6 w-6"}),title:"Equipo de Seguridad",description:o?"Personal de apoyo, salvavidas y asistencia de primeros auxilios estarán presentes durante el desarrollo de la competencia.":"Salvavidas certificados, embarcaciones de apoyo, paramédicos y ambulancia estarán presentes durante todo el evento."}],w=a.distances.map(s=>({distance:s.label,groups:s.categories.map(r=>r.label)})),$=["Gorro de natación","Traje de baño adecuado","Gafas de natación (recomendadas)"],S=o?["Material de propulsión (aletas, paletas, snorkel, etc.)","Dispositivos de flotación artificiales","Salida antes de la señal oficial","Interferir con otro nadador o cambiarse de carril sin autorización"]:["Material de propulsión (aletas, paletas, etc.)","Dispositivos de flotación artificiales","Aparatos electrónicos no autorizados","Elementos que puedan afectar la seguridad"],p=o?["Salvavidas o personal de apoyo en el área de piscina","Primeros auxilios disponibles","Control de salida y llegada por parte de la organización","Área de competencia supervisada"]:["Salvavidas certificados","Embarcaciones de apoyo","Paramédicos en meta","Ambulancia disponible"],C=o?"Condiciones de Piscina y Competencia":"Condiciones Climáticas y del Agua",E=o?"La organización se reserva el derecho de modificar el orden de pruebas, series o carriles por razones operativas, seguridad o desarrollo del evento.":"La organización se reserva el derecho de suspender, posponer o modificar el evento en caso de condiciones climáticas adversas o calidad del agua que comprometan la seguridad de los participantes.",L=o?"El área de piscina, orden de pruebas y condiciones de competencia serán supervisadas durante todo el evento. Cualquier cambio será comunicado a los participantes.":"Las condiciones del lago y el clima serán monitoreadas constantemente. Cualquier cambio o decisión será comunicada a los participantes con la mayor anticipación posible.",i=s=>String(s??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;"),M=()=>{const s=window.open("","_blank");if(!s){alert("Permite ventanas emergentes en tu navegador para generar el PDF.");return}const x=new Date().toLocaleString("es-HN",{dateStyle:"medium",timeStyle:"short"}),h=`<div class="logo"><img src="${K}" alt="Swim Plus" /></div>`,H=y.map(t=>`
      <div class="rule">
        <h3>${i(t.title)}</h3>
        <p>${i(t.description)}</p>
      </div>
    `).join(""),k=w.map(t=>`
      <section class="category">
        <h3>${i(t.distance)}</h3>
        <ul>
          ${t.groups.map(B=>`<li>${i(B)}</li>`).join("")}
        </ul>
      </section>
    `).join(""),F=$.map(t=>`<li>${i(t)}</li>`).join(""),O=S.map(t=>`<li>${i(t)}</li>`).join("");s.document.write(`
      <!DOCTYPE html>
      <html lang="es">
        <head>
          <meta charset="utf-8" />
        ...
        <body>
          ...
          <div class="event-info">
            <dl>
              ...
              <dt>Inscripción:</dt><dd>${i(N)} ${i(a.paymentInfo)}</dd>
            </dl>
          </div>
          <div class="section">
